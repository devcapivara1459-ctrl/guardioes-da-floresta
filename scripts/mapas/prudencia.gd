extends "res://scripts/mapas/floresta.gd"
...
func _ready() -> void:
	...
	curupira = preload("res://scripts/batalhas/curupira_sprite.gd").new()
...
func _salvar() -> void:
	...
	aviso_save.text = "" if progresso.salvar("res://cenas/capitulos/prudencia/prudencia.tscn",jogador.position) else progresso.erro
...
func _avancar() -> void:
	...
	if not falando and encontro_pendente:
		...
		curupira.modulate = Color.WHITE
		curupira.show()
		var batalha := preload("res://scripts/batalhas/batalha.gd").new()
		add_child(batalha)
		batalha.concluida.connect(_fim_batalha)
		batalha.iniciar(jogador,curupira,preload("res://scripts/batalhas/curupira_dados.gd").criar(),int(progresso.dados.ervas))
